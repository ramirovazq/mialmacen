...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):        
        with open(settings.BASE_DIR + '/load_init/posiciones.csv') as csvfile_in:
            ...
            for indice, row in enumerate(readCSV):
                if indice != 0: # quit name of column
                    ...
                    codigo = Posicion.siguiente_codigo()
                    obj, bandera = Posicion.objects.get_or_create(nombre=nombre, defaults={'codigo': codigo})
                    if bandera:
