...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):        
        with open(settings.BASE_DIR + '/load_init/all_medidas_unique.csv') as csvfile_in:
            ...
            for indice, row in enumerate(readCSV):
                if indice != 0: # quit name of column
                    ...
                    try:
                        Medida.objects.get(nombre=nombre)
                        #print("{}".format(nombre))
                    except Medida.DoesNotExist:
                        #print("NO...;{}".format(nombre))
                        self.stdout.write(self.style.ERROR('Medida no existe: {} '.format(nombre)))
